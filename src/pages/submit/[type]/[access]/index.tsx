...
export default function SubmitAccess() {
  ...
  const handleSubmit = (e) =>{
    if(inputs.area && inputs.title && inputs.description && (router.query.type as string !== "evaluation")){
        session.status === "authenticated" ? mutationLoggedIn.mutate({...inputs, type: router.query.type as string}) : mutation.mutate({...inputs, type: router.query.type as string})
    } else if(inputs.area && inputs.title && inputs.description && count > 0){
        session.status === "authenticated" ? mutationLoggedIn.mutate({...inputs, type: router.query.type as string, rating: count}) : mutation.mutate({...inputs, type: router.query.type as string, rating: count})
    }
  }
  // HOOKS
  ...
  return (
    <>
      ...
      <div className=" mt-20 mb-3 flex h-auto w-auto flex-wrap justify-center rounded-xl bg-white p-5 md:w-1/2">
        ...
        <div className="flex w-full flex-col">
          <div className="m-3 mb-1 flex w-full flex-col">
            <p className="m-1 mb-1 font-extrabold">{translation("Mandatory")}</p>
          ...
          <div className="m-3 mb-1 flex w-full flex-col">
            ...
            <div className={"relative block w-full"}>
              <textarea
                name="description"
                onChange={handleChange}
                className="w-full rounded-md border-2 border-kfupm-dark-green border-opacity-60 p-3 outline-none transition focus:bg-opacity-50"
              />
            </div>
          </div>
          <div className="m-3 mt-1 mb-1 flex w-full flex-col">
        ...
        <Button className="m-3 w-full bg-kfupm-dark-green py-3 text-white" onClick={handleSubmit}>
          {translation("Button")}
        </Button>
      </div>
    </>
